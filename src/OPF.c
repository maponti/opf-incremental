...
void opf_OPFTraining(Subgraph *sg){
  ...
  // goes through the nodes inserting the prototypes in a heap
  for (p = 0; p < sg->nnodes; p++) {
    if (sg->node[p].status==opf_PROTOTYPE){
      //printf("Node %d (%.2f,%.2f)[%d] is a prototype!\n", p, 
      //  sg->node[p].feat[0], sg->node[p].feat[1], sg->node[p].truelabel);
      sg->node[p].pred   = NIL;
      pathval[p]         = 0;
  ...
  while ( !IsEmptyRealHeap(Q) ) {
    // gets a node from the heap (on the first iteration it is a prototype)
    // heap is ordered by pathval
    RemoveRealHeap(Q,&p);

    // include it in the ordered list and update pathval
    sg->ordered_list_of_nodes[i]=p; i++;
    sg->node[p].pathval = pathval[p];

    for (q=0; q < sg->nnodes; q++){
      if (p!=q){
        // if pathval of the node removed from the heap is lower than the current node
      	if (pathval[p] < pathval[q]){

                // compute weight between p and q
          ...
          // gets the maximum between the pathval of p and computed weight
          // obs: if p is a prototype its pathval will be 0
      	  tmp  = MAX(pathval[p],weight);

                // check if the weight is lower than the pathval of q
                // obs: if q is been tested for the first time, it will be FLT_MAX (infty) 
      	  if ( tmp < pathval[ q ] ) {
                  // then it is conquested by the predecessor 'p'
      	    sg->node[q].pred  = p;
      	    sg->node[q].label = sg->node[p].label; // get the same label of 'p'
      	    UpdateRealHeap(Q, q, tmp); // insert the node on the heap with the current lowest weight

            // ** OPFI **-------------------------------------------
            // after each conquest, updates the adjacency list of the conqueror node - if they are of same class
            if (sg->node[p].truelabel == sg->node[q].truelabel){
             // printf("\tInserting as adjacents nodes %d (%.2f,%.2f)[%d] and %d (%.2f,%.2f)[%d]\n",
              //  p, sg->node[p].feat[0], sg->node[p].feat[1], sg->node[p].truelabel,
              //  q, sg->node[q].feat[0], sg->node[q].feat[1], sg->node[q].truelabel);
              InsertSet(&(sg->node[p].incAdj),q);
              // and of the conquered node
              InsertSet(&(sg->node[q].incAdj),p);
            }
      	  }
	       }
      }
...
void opf_OPFClassifying(Subgraph *sgtrain, Subgraph *sg)
{
	...
	// percorre todos os nodes do conjunto-teste, classificando-os
...
void opf_WriteModelFile(Subgraph *g, char *file){
  ...
  for (i = 0; i < g->nnodes; i++){
    ...
    // OPFI CODE -----------------------
    fwrite(&g->node[i].prototypePair, sizeof(int), 1, fp);

    int size = GetSetSize(g->node[i].incAdj);
    int* adj = setToArray(g->node[i].incAdj);
    fwrite(&size, sizeof(int), 1, fp);
    fwrite(adj, sizeof(int), size, fp);
    // ---------------------------------
...
Subgraph *opf_ReadModelFile(char *file){
  ...
  /*reading df*/
  fread(&g->df, sizeof(float), 1, fp);

  // for supervised opf by pdf

  fread(&g->K, sizeof(float), 1, fp);
  fread(&g->mindens, sizeof(float), 1, fp);
  fread(&g->maxdens, sizeof(float), 1, fp);
  ...
  for (i = 0; i < g->nnodes; i++){
    ...
    fread(&g->node[i].pred, sizeof(int), 1, fp);
    fread(&g->node[i].label, sizeof(int), 1, fp);
    fread(&g->node[i].pathval, sizeof(float), 1, fp);
    fread(&g->node[i].radius, sizeof(float), 1, fp);

    // OPFI CODE -----------------------
    fread(&g->node[i].prototypePair, sizeof(int), 1, fp);

    int size, *adj;
    fread(&size, sizeof(int), 1, fp);
    adj = (int*)malloc(sizeof(int)*size);
    fread(adj, sizeof(int), size, fp);
    g->node[i].incAdj = arrayToSet(adj, size);
    //----------------------------------
  ...
  for (i = 0; i < g->nnodes; i++){
    fread(&g->ordered_list_of_nodes[i], sizeof(int), 1, fp);
  }
    ...
    for (q=0; q < sg->nnodes; q++){
      if (Q->color[q]!=BLACK){
      	if (p!=q){
            ...
            // ** OPFI **-------------------------------------------
            if (sg->node[q].status != opf_PROTOTYPE)sg->node[q].prototypePair=NIL;     
