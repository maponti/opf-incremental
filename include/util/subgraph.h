...
typedef struct _snode {
  ...
  int prototypePair; // node that is a prototype pair (i.e., if this node is a prototype, gets the corresponding prototype of another class) opf-incremental
  Set   *incAdj; // adjacency list for opf-incremental
} SNode;
...
typedef struct _index_path_pair{
  int index;
  float pathval;
} IndexPathPair;
...
void WriteSubgraphPrototype(Subgraph *g, char *file); //write subgraph to disk
...
void AddSNode(Subgraph *sg, SNode *newSNode); // Adds a new node
void correctList(Subgraph *sg);// Corrects the list of pathvalues
void sortList(Subgraph *sg);// Sorts the list of pathvalues
