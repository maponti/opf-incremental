...
for base in bases:
	...
	for iteration in iterations:
		#accFile = open(base+str(iteration)+"/acc100.txt", "r")
	...
	#	DSTDEVaccuracies.append(statistics.stdev(meanDAccList))
		...
		RTMEAN_times.append(statistics.mean(meanRTTimeList))
		RTSTDEVtimes.append(statistics.stdev(meanRTTimeList))
		DMEAN_times.append(statistics.mean(meanDTimeList))
		DSTDEVtimes.append(statistics.stdev(meanDTimeList))
	...
	outTimeFile.write("mean_timeI,mean_timeRT,mean_timeD,stdev_timeI,stdev_timeRT,stdev_timeD\n")
	for i in range(len(IMEAN_times)):
		outTimeFile.write(str(IMEAN_times[i])+","+str(RTMEAN_times[i])+","+str(DMEAN_times[i])+","
						+str(ISTDEVtimes[i])+","+str(RTSTDEVtimes[i])+","+str(DSTDEVtimes[i])+"\n")
